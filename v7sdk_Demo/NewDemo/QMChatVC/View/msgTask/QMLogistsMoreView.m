...
- (void)setSubViews {
    CGRect frm = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height, self.frame.size.width, [[UIScreen mainScreen] bounds].size.height - 220*kScale6);
...
//    [header addSubview:lab];
    ...
    _headerLab.textColor = [UIColor colorWithRed:135/255.0 green:135/255.0 blue:135/255.0 alpha:1.0];
...
- (void)closeAction {
    CGRect frm = self.bgView.frame;
    frm.origin.y = [[UIScreen mainScreen] bounds].size.height;
    [UIView animateWithDuration:0.25 animations:^{
        self.bgView.frame = frm;
    } completion:^(BOOL finished) {
