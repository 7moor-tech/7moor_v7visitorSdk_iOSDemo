...
@interface QMSegmentView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _titleSelectedColor = QMThemeManager.shared.mainColorModel.color;
...
//    _needScrollToSelectedItem = YES;
//    [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.originalIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:NO];
}
...
- (void)setVernierHeight:(CGFloat)vernierHeight {
    if (self.vernier && self.vernier.superview) {
        [self.vernier mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (UIView *)bottomBorder {
    if (!_bottomBorder) {
        ...
        _bottomBorder.backgroundColor = k_QMRGB(237, 237, 237);
    }
